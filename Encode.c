...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
	...
	//validating for secret file
	if ( argv[3] != NULL )
	{
		//getting secret file extension 
		char *Extension = strstr(argv[3],".");

		if ( Extension != NULL )
		{
			//comparing secret file extension
			if ( !strcmp(Extension,".txt") || !strcmp(Extension,".c") || !strcmp(Extension,".sh") )
			encInfo -> secret_fname = argv[3];

			//printing an error if invalid file extension is passed
			else
			{
	...
	//printing an error if the Secret file is not passed
	else
	{
	 	 printf("USAGE: Secret File Format Should be <.txt file> or <.sh file> or <.c file>\n");	 
		 printf("ERROR: Usage!\n");
	...
	{	
		...
		{
		 	printf("ERROR: Output Image File Format Should be <.bmp file>\n");
		 	printf("ERROR: Usage!\n");
         	printf("USAGE: Encoding: %s -e <.bmp file> <.txt file> [output file]\n",argv[0]);	
		 	return e_failure;
		}
...
Status copy_remaining_img_data(EncodeInfo *encInfo)
{
	...
	size_of_src_image = encInfo -> image_capacity + 54 + 1;
