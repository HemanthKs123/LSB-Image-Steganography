...
Status read_and_validate_decode_args(char *argv[],DecodeInfo *decInfo)
{
	...
	//validating for decoded file
	else if ( argv[3] != NULL )
    {
        //getting decoded file extension
        char *Extension = strstr(argv[3],".");

        if ( Extension != NULL )
        {
            //comparing decoded file extension
	...
	if ( open == e_success )
	{
		...
		if ( check_magic == e_success )
		{
			...
			else
			{
				...
				printf("ERROR: You May Have Provided Different Format for Secret File in Encoding, Please Verify\n");
				return e_failure;
			}
		}
